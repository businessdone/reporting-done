...
async def create_log(log: LogCreateModel, session: AsyncSession) -> LogResponseModel:
    """
    Create a new log in the database.
    """
    task_repo = Repository(session, Task)
    task = await task_repo.get(id=log.task_id)

    user_repo = Repository(session, User)
    user = await user_repo.get(id=log.user_id)
...
async def get_log(session: AsyncSession, **kwargs) -> LogResponseModel:
    """
    Retrieve a single log from the database based on provided criteria.
    """
    repo = Repository[Log](session, Log)
    log = await repo.query(**kwargs)
    if not log:
...
async def upsert_log(log: LogResponseModel, session: AsyncSession) -> LogResponseModel:
    """
    Insert a new log or update an existing log based on unique constraints.
    """
    repo = Repository[Log](session, Log)
    existing_log = await repo.get(id=log.id)
    if existing_log:
...
async def get_projects_with_recent_logs() -> Dict[Project, List[Log]]:
    ...
    """
    session = await get_session()  # type: ignore[call-arg]
    ...
    recent_logs = await log_repository.query(created_at__gte=past_24h)
    ...
    projects = await project_repository.query(in_={Project.id: project_ids})
    ...
    projects_map: Dict[str, Project] = {
        str(project.id): project for project in projects
    }
